...
export class DocsService {
  ...
  public getText = async ({ documentId, tabId }: { documentId: string; tabId?: string }) => {
    ...
    try {
      ...
      const tabs = (res as any).data?.tabs || [];
  ...
  public replaceText = async ({
    ...
    try {
      ...
      const tabs = (docBefore as any).data?.tabs || [];
          ...
          const adjustedRequest: docs_v1.Schema$Request = {
            updateTextStyle: {
              ...
              range: {
                tabId: tabId,
                startIndex:
                  (formatRequest.updateTextStyle.range?.startIndex || 0) + adjustedPosition,
    ...
    return requests.map((req) => {
      ...
      if (newReq.updateTextStyle?.range) {
        newReq.updateTextStyle = {
          ...newReq.updateTextStyle,
          range: { ...newReq.updateTextStyle.range, tabId: tabId } as any,
        };
      }
      if (newReq.updateParagraphStyle?.range) {
        newReq.updateParagraphStyle = {
          ...newReq.updateParagraphStyle,
          range: { ...newReq.updateParagraphStyle.range, tabId: tabId } as any,
        };
      }
      if (newReq.insertText?.location) {
        newReq.insertText = {
          ...newReq.insertText,
          location: { ...newReq.insertText.location, tabId: tabId } as any,
        };
      }
      return newReq;
    });

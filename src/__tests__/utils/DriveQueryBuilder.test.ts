...
describe('DriveQueryBuilder', () => {
  describe('buildDriveSearchQuery', () => {
    it('should build fullText query for regular search', () => {
      ...
      expect(query).toBe(
        "mimeType='application/vnd.google-apps.document' and fullText contains 'test query'"
      );
    });
    ...
    it('should build name query for title-prefixed search', () => {
      ...
      expect(query).toBe(
        "mimeType='application/vnd.google-apps.presentation' and name contains 'My Presentation'"
      );
    });
    ...
    it('should handle quoted title searches', () => {
      ...
      expect(query).toBe(
        "mimeType='application/vnd.google-apps.spreadsheet' and name contains 'Budget 2024'"
      );
    });
    ...
    it('should handle single-quoted title searches', () => {
      ...
      expect(query).toBe(
        "mimeType='application/vnd.google-apps.document' and name contains 'Q4 Report'"
      );
    });
    ...
    it('should escape special characters in query', () => {
      ...
      expect(query).toBe(
        "mimeType='application/vnd.google-apps.document' and fullText contains 'test\\'s query\\\\path'"
      );
    });
    ...
    it('should escape special characters in title search', () => {
      const query = buildDriveSearchQuery(
        ...
        "title:John's Presentation\\2024"
      );
      expect(query).toBe(
        "mimeType='application/vnd.google-apps.presentation' and name contains 'John\\'s Presentation\\\\2024'"
      );
    });
    ...
    it('should handle empty strings', () => {
      ...
      expect(query).toBe(
        "mimeType='application/vnd.google-apps.spreadsheet' and fullText contains ''"
      );
    });
    ...
    it('should handle title prefix with whitespace', () => {
      ...
      expect(query).toBe(
        "mimeType='application/vnd.google-apps.presentation' and name contains 'My Doc'"
      );
    });

    it('should work with all MIME types', () => {
      expect(buildDriveSearchQuery(MIME_TYPES.DOCUMENT, 'test')).toContain(
        'application/vnd.google-apps.document'
      );
      expect(buildDriveSearchQuery(MIME_TYPES.PRESENTATION, 'test')).toContain(
        'application/vnd.google-apps.presentation'
      );
      expect(buildDriveSearchQuery(MIME_TYPES.SPREADSHEET, 'test')).toContain(
        'application/vnd.google-apps.spreadsheet'
      );
      expect(buildDriveSearchQuery(MIME_TYPES.FOLDER, 'test')).toContain(
